...
from climatiq.core.controller import Controller, ActionType, ControlAction
...
def test_gradual_nudge_small_adjustment(controller, mock_units):
    ...
    if action.action_type == ActionType.ADJUST_TEMP:
        temp_change = abs(action.parameters['temperature'] - action.parameters.get('previous', 21.0))
...
def test_high_variance_triggers_action(controller, mock_units):
    ...
    action = controller.decide_action(
        status,
        {'cycling_predicted': True, 'probability': 0.8},
        analysis
    )
