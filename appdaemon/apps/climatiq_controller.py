...
class ClimatIQController(hass.Hass):
    ...
    def initialize(self):
        ...
        self.log(f"Controller gestartet (Interval: {interval}min, Räume: {list(self.rooms.keys())})")
    ...
    def control_cycle(self, kwargs):
        ...
        try:
            ...
            self.log(f"Power: {state['power']:.0f}W | Outdoor: {state['outdoor_temp']:.1f}°C | Δ Total: {state['total_delta_abs']:.1f}K")
            
            # Check: Sind wir in instabiler Zone?
            in_unstable = self._is_in_unstable_zone(state['power'])
            if in_unstable:
                self.log(f"⚠️ Instabile Zone ({state['power']:.0f}W) - keine Actions", level="WARNING")
    ...
    def decide_actions(self, state: Dict) -> List[Dict]:
        ...
        if len(actions) > max_actions:
            ...
            actions = sorted(actions, key=lambda a: abs(state['rooms'][a['room']]['delta']), reverse=True)
    ...
    def execute_action(self, action: Dict):
        ...
        self.log(f"→ {room_name}: {action['old_target']:.1f}°C → {new_target:.1f}°C ({action['reason']})")
