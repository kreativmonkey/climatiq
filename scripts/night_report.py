...
def analyze_period(p_start, p_end, label):
    ...
    target_zone_pct = (df['is_target_zone'].sum() / total_on_minutes) * 100 if total_on_minutes > 0 else 0
...
print(f"Stabilität (Instabil %): {current_report['unstable_pct']:.1f}% (Gestern: {yesterday_report['unstable_pct']:.1f}% if yesterday_report else 'N/A')")
