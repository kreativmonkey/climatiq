...
if stable:
    for r in stable[:3]:
        ...
        print(
            f"  • {avg:.0f}W (±{(r.power_range[1]-r.power_range[0])/2:.0f}W): score={r.stability_score:.2f}, σ={std:.0f}W, n={r.sample_count}"
        )
else:
...
controller = Controller(
    {"comfort": {"target_temp": 21.0, "night_temp": 19.0}, "unit_priorities": {}}
)
