...
if delta_cols:
    df["total_delta_abs"] = df[[f"{c}_abs" for c in delta_cols if f"{c}_abs" in df.columns]].sum(
        axis=1
    )
...
ax1.barh(
    range(len(top_features)), top_features["importance"] * 100, color=colors_importance, alpha=0.7
)
...
df_ml[df_ml["target"] == 0][top_feature].hist(
    bins=30, alpha=0.6, color="green", label="Stable", ax=ax3
)
df_ml[df_ml["target"] == 1][top_feature].hist(
    bins=30, alpha=0.6, color="red", label="Unstable", ax=ax3
)
...
if "total_delta_abs" in df_ml.columns:
    df_ml[df_ml["target"] == 0]["total_delta_abs"].hist(
        bins=30, alpha=0.6, color="green", label="Stable", ax=ax5
    )
    df_ml[df_ml["target"] == 1]["total_delta_abs"].hist(
        bins=30, alpha=0.6, color="red", label="Unstable", ax=ax5
    )
...
feature_importance_df.to_csv(
    "/home/diyon/.openclaw/workspace/climatiq/data/feature_importance_causal.csv", index=False
)
