...
for i in range(2):
    for j in range(2):
        text = ax3.text(j, i, cm[i, j], ha="center", va="center", color="white" if cm[i, j] > cm.max()/2 else "black", fontsize=20)
...
ax5.scatter(df_ml[top_feature], df_ml['power_std'], c=df_ml['target'], cmap='RdYlGn_r', alpha=0.3, s=10)
...
for idx, row in cluster_df.iterrows():
    ...
    ax6.scatter(row['power_median'], row['power_std_median'], 
                s=row['size']/2, alpha=0.7, color=color,
...
# Export
feature_importance_df.to_csv('/home/diyon/.openclaw/workspace/climatiq/data/feature_importance_gmm.csv', index=False)
