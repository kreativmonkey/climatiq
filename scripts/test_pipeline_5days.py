...
for _, obs in obs_df[obs_df["risk"] > 0.6].iterrows():
    ...
    if action.action_type != ActionType.NO_ACTION:
        ...
        action_details.append(
            {"time": obs["time"], "type": action.action_type.value, "reason": action.reason[:60]}
        )
...
print(
    f"  • <700W min stable: Target vs {result.min_stable_power:.0f}W {'✅' if result.min_stable_power < 700 else '❌'}"
)
