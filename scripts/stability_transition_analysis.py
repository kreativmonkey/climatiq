...
if res_df["units_before"].max() > 0 or res_df["units_after"].max() > 0:
    ax2.hist(
        res_df["units_before"].dropna(), bins=10, alpha=0.7, label="Vor Übergang", color="green"
    )
...
if "outdoor_before" in res_df.columns:
    ax3.scatter(
        res_df["outdoor_before"], res_df["power_before"], alpha=0.6, c="green", label="Stabil", s=50
    )
    ax3.scatter(
        res_df["outdoor_after"], res_df["power_after"], alpha=0.6, c="red", label="Instabil", s=50
    )
...
if len(stable_df) > 0:
    ...
    print(
        f"  Dauer: {stable_df['duration_min'].mean():.0f} min (min: {stable_df['duration_min'].min():.0f}, max: {stable_df['duration_min'].max():.0f})"
    )
