...
for c in range(best_n):
    ...
    stats = {
        ...
        "outdoor_temp_mean": (
            cluster_data["outdoor_temp"].mean() if "outdoor_temp" in cluster_data else np.nan
        ),
    }
...
for pmin, pmax in power_bins:
    ...
    if mask.sum() > 100:
        ...
        print(
            f"  {pmin:4d}-{pmax:4d}W: Threshold={threshold:.1f}W, Stabil: {stable_count}/{mask.sum()} ({stable_count/mask.sum()*100:.1f}%)"
        )
...
scatter1 = ax1.scatter(
    df["power"], df["power_std"], c=df["cluster_gmm"], cmap="viridis", alpha=0.3, s=5
)
...
ax3.scatter(
    df[df["stable_quantile"]]["power"],
    df[df["stable_quantile"]]["power_std"],
    c="green",
    alpha=0.3,
    s=5,
    label="Stabil (25% Quantil)",
)
ax3.scatter(
    df[~df["stable_quantile"]]["power"],
    df[~df["stable_quantile"]]["power_std"],
