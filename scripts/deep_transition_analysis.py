...
df['active_units'] = sum(df[col].fillna(0).astype(bool).astype(int) for col in unit_cols if col in df.columns)
...
ax3 = axes[1][0]
if 'outdoor_before' in res_df.columns:
    ax3.scatter(res_df['outdoor_before'], res_df['power_before'], alpha=0.5, c='green', label='Stabil')
    ax3.scatter(res_df['outdoor_after'], res_df['power_after'], alpha=0.5, c='red', label='Instabil')
